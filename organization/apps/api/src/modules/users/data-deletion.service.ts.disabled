...
export class DataDeletionService {
  ...
  private async softDelete(userId: string): Promise<DeletionResult> {
    ...
    await this.prisma.user.update({
      where: { id: userId },
      data: {
        email: `deleted_${userId}@citadelbuy.deleted`,
        name: 'Deleted User',
        password: 'DELETED',
  ...
  private async anonymizeUser(userId: string): Promise<DeletionResult> {
    const deletedAt = new Date();
    const anonymousEmail = `anonymous_${userId}@citadelbuy.anonymized`;
    ...
    for (const order of orders) {
      ...
      try {
        ...
        if (shippingAddress) {
          ...
          shippingAddress.email = 'anonymous@citadelbuy.com';
      ...
      await prisma.order.update({
        ...
        data: {
          ...
          guestEmail: order.guestEmail ? 'anonymous@citadelbuy.com' : null,
